...
class JWT {
        ...
        static async initKeys(){
                ...
                const keys = await keyFile.json() as {publicKey:JWK,privateKey:JWK}
                if(keys.publicKey && keys.privateKey){
                        this.privateKey = (await importJWK(keys.privateKey,"RS256")) as keyType;
                        this.publicKey = (await importJWK(keys.publicKey,"RS256")) as keyType;
                }
                else{
                        this.genNewKeys()
                }
        ...
        static async verify(token: string):Promise<JWT_PAYLOAD>{
                const res  =  (await jwtVerify(token,this.publicKey)).payload.payload as JWT_PAYLOAD
                return res
        }
