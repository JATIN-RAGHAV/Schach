...
import { anonymousSocketPlugin } from './plugins/anonymouseSocket';

export const gameRun = new Elysia().use(anonymousSocketPlugin)
.ws('/game/run', {
    ...
    async open(ws) {
        ...
        console.log(
            `An Anonymous wants to play a Game as ${colorType[color]},time:${time},increment:${increment}.`,
        );

        // Check if another player exists for the same kind of game with the right color
        const opponentResponse = getOpponent(color, time, increment,true);
        if (opponentResponse) {
            // Decide who is going to play as white and who is going to play as black
            let whiteSocket = currentPlayer.ws as ElysiaWS;
            let blackSocket = opponentResponse.oppo.ws as ElysiaWS;
